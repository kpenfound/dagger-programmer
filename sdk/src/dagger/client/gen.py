...
class ModuleHelperID(Scalar):
    """The `ModuleHelperID` scalar type represents an identifier for an
    object of type ModuleHelper."""
...
class ModuleWorkspaceID(Scalar):
    """The `ModuleWorkspaceID` scalar type represents an identifier for an
    object of type ModuleWorkspace."""
...
class EnumTypeDef(Type):
    ...
    async def values(self) -> list["EnumValueTypeDef"]:
        ...
        _ids = await _ctx.execute(list[Response])
        return [EnumValueTypeDef(Client.from_context(_ctx)._select("loadEnumValueTypeDefFromID", [Arg("id", v.id)],)) for v in _ids]
...
class Error(Type):
    ...
    async def values(self) -> list["ErrorValue"]:
        ...
        class Response:
            id: ErrorValueID
        _ids = await _ctx.execute(list[Response])
        return [ErrorValue(Client.from_context(_ctx)._select("loadErrorValueFromID", [Arg("id", v.id)],)) for v in _ids]
...
class ModuleHelper(Type):
    ...
    async def get_module_schema(self, mod: Module) -> str:
        ...
        _ctx = self._select("getModuleSchema", _args)
        return await _ctx.execute(str)

    async def get_sdk_main_file(self, sdk: str, name: str) -> str:
        """
        ...
        _ctx = self._select("getSdkMainFile", _args)
        return await _ctx.execute(str)
    ...
    async def get_type_name(self, type: "TypeDef") -> str:
        ...
        _ctx = self._select("getTypeName", _args)
        return await _ctx.execute(str)

    async def id(self) -> ModuleHelperID:
        """A unique identifier for this ModuleHelper.
        ...
        _args: list[Arg] = []
        _ctx = self._select("id", _args)
        return await _ctx.execute(ModuleHelperID)
...
class ModuleSource(Type):
    ...
    async def dependencies(self) -> list["ModuleSource"]:
        ...
        class Response:
            id: ModuleSourceID
        _ids = await _ctx.execute(list[Response])
        return [ModuleSource(Client.from_context(_ctx)._select("loadModuleSourceFromID", [Arg("id", v.id)],)) for v in _ids]
    ...
    async def sync(self) -> Self:
        ...
        _ctx = Client.from_context(_ctx)._select("loadModuleSourceFromID", [Arg("id", _id)])
        return ModuleSource(_ctx)
...
@typecheck
class ModuleWorkspace(Type):

    async def get_examples_reference(self) -> str:
        """
        Get reference information on writing an example module for a dagger
        module
        ...
        """
        _args: list[Arg] = []
        _ctx = self._select("getExamplesReference", _args)
        return await _ctx.execute(str)

    async def get_sdk_reference(self, sdk: str) -> str:
        """
        ...
        ----------
        sdk:
            Dagger SDK language to get reference for
        ...
        _ctx = self._select("getSdkReference", _args)
        return await _ctx.execute(str)

    async def id(self) -> ModuleWorkspaceID:
        """A unique identifier for this ModuleWorkspace.
        ...
        -------
        ModuleWorkspaceID
            The `ModuleWorkspaceID` scalar type represents an identifier for
            an object of type ModuleWorkspace.
        ...
        _args: list[Arg] = []
        _ctx = self._select("id", _args)
        return await _ctx.execute(ModuleWorkspaceID)
    ...
    def with_(self, cb: Callable[["ModuleWorkspace"], "ModuleWorkspace"]) -> "ModuleWorkspace":
        """Call the provided callable with current ModuleWorkspace.
...
class Client(Root):
    ...
    def load_module_helper_from_id(self, id: ModuleHelperID) -> ModuleHelper:
        """Load a ModuleHelper from its ID."""
        _args = [
            Arg("id", id),
        ]
        _ctx = self._select("loadModuleHelperFromID", _args)
        return ModuleHelper(_ctx)
    ...
    def load_module_workspace_from_id(self, id: ModuleWorkspaceID) -> ModuleWorkspace:
        """Load a ModuleWorkspace from its ID."""
        _args = [
            Arg("id", id),
        ]
        _ctx = self._select("loadModuleWorkspaceFromID", _args)
        return ModuleWorkspace(_ctx)
    ...
    def module_workspace(self, sdk: str, name: str, *, dependencies: list[Directory] | None = None, dagger_version: str = "latest", workspace: Directory | None = None,) -> ModuleWorkspace:
        _args = [
            Arg("sdk", sdk),
            Arg("name", name),
            Arg("dependencies", dependencies, None),
            Arg("daggerVersion", dagger_version, "latest"),
